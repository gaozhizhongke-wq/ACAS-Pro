...
def _try_statsforecast(
    values: np.ndarray, horizon: int, season_length: int = 7
) -> Optional[Dict]:
    """
    ...
    try:
        ...
        return {
            ...
            "lower": [max(0, lo) for lo in lower],
            "upper": upper,
        }
    except (
        sqlite3.Error,
...
class TimesFMEngine:
    ...
    def __init__(self):
        ...
        logger.info(
            f"TimesFM Engine initialized (StatsForecast available: {self.statsforecast_ok})"
        )
            ...
            if self._STATUS_FILE.exists():
                ...
                if data.get("available", True) is False:
                    last_fail = datetime.fromisoformat(
                        data.get("last_failure", "2000-01-01")
                    )
    ...
    def forecast(
        ...
        if len(historical_data) < 14:
            logger.warning(
                f"Insufficient data for {product_id}: {len(historical_data)} points"
            )
            return self._generate_fallback_forecast(
                product_id, historical_data, horizon_days
            )
        ...
        if forecast_values is None:
            forecast_values = self._holt_winters_forecast(
                values.tolist(), horizon_days, has_seasonality
            )
            ...
            std_error = (
                stdev(residuals) if len(residuals) > 1 else mean(residuals) * 0.1
            )

            lower_bounds = [
                max(0, v - z_score * std_error * (1 + i * 0.02))
                for i, v in enumerate(forecast_values)
            ]
            upper_bounds = [
                v + z_score * std_error * (1 + i * 0.02)
                for i, v in enumerate(forecast_values)
            ]
    ...
    def _holt_winters_forecast(
        self, values: List[float], horizon: int, use_seasonality: bool
    ) -> List[float]:
        ...
        for i, actual in enumerate(values):
            ...
            # Update level
            new_level = self.alpha * (actual - seasonal[s_idx]) + (1 - self.alpha) * (
                level + trend
            )
