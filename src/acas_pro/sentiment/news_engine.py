...
class MarketIntelligenceEngine:
    ...
    def fetch_intelligence(
        ...
        if regions:
            filtered = [
                a for a in filtered if any(r in a.affected_regions for r in regions)
            ]
    ...
    def _generate_sample_data(self, count: int) -> List[NewsArticle]:
        ...
        for i in range(count):
            ...
            if sentiment.overall_sentiment in [
    ...
    def _get_recommended_actions(self, risk_type: str) -> List[str]:
        ...
        actions = {
            ...
            "price_volatility": ["审查定价策略", "考虑对冲策略", "与供应商重新谈判"],
            "geopolitical": ["监控政策变化", "评估替代市场", "咨询法律团队"],
            "natural_disaster": ["激活应急预案", "评估库存位置", "联系保险公司"],
            "cybersecurity": ["审查安全协议", "进行安全审计", "通知相关方"],
        }
    ...
    def get_sentiment_summary(self, articles: List[NewsArticle]) -> Dict:
        ...
        return {
            ...
            "positive_ratio": round(
                (distribution["very_positive"] + distribution["positive"])
                / len(articles),
                2,
            ),
            "negative_ratio": round(
                (distribution["very_negative"] + distribution["negative"])
                / len(articles),
                2,
            ),
            "trend": trend,
