...
class SettlementEngine:
    ...
    def create_from_template(
        ...
        **kwargs,
        ...
        for i, party_config in enumerate(party_configs):
            template_party = (
                template["parties"][i] if i < len(template["parties"]) else {"share": 0}
            )
    ...
    def get_settlements_by_source(
        self, source_id: str, source_type: Optional[str] = None
    ...
    def execute_settlement(self, settlement_id: str) -> Dict[str, Any]:
        ...
        record.blockchain_tx_hash = (
            results["transactions"][0]["tx_hash"] if results["transactions"] else None
        )
        self._save_settlement(record)
    ...
    def get_settlement_statistics(
        self, start_date: str, end_date: str, party_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """获取结算统计"""
        query = """
        ...
        return {
            ...
            "total_amount": total_amount,
            "completed": completed,
            "completion_rate": completed / total_settlements
