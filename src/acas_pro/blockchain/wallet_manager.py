...
class WalletManager:
    ...
    SUPPORTED_TOKENS = {
        'USDT': {'chain': 'ethereum', 'contract': '0xdAC17F958D2ee523a2206206994597C13D831ec7'},
        'USDC': {'chain': 'ethereum', 'contract': '0xA0b86a33E6441e0A421e56E4773C3C4b0Db7E5f0'},
        'DAI': {'chain': 'ethereum', 'contract': '0x6B175474E89094C44Da98b954EedeAC495271d0F'},
    }
    ...
    def _save_transaction(self, tx: Transaction) -> None:
        ...
        """, (
            tx.id, tx.tx_type.value, tx.from_wallet, tx.to_wallet,
            tx.amount, tx.currency, tx.fee, tx.status.value,
            tx.blockchain_tx_hash, tx.block_number, tx.confirmations,
            tx.settlement_id, tx.description, tx.created_at, tx.confirmed_at
        ))
    ...
    def _row_to_transaction(self, row: Dict[str, Any]) -> Transaction:
        ...
        return Transaction(
            ...
            fee=row['fee'] or 0.0,
            status=TransactionStatus(row['status']) if row['status'] else TransactionStatus.PENDING,
