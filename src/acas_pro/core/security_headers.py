...
class SecurityHeaders:
    ...
    # Legacy CSP — removed unsafe-inline/unsafe-eval. All templates must support nonce.
    # Production deployments MUST set use_nonce=True (default).
    LEGACY_CSP = (
        "default-src 'self'; "
        ...
        def add_security_headers(response) -> None:
            ...
            if self.use_nonce:
                ...
                _logging.getLogger(__name__).warning(
                    "Legacy CSP (no nonce) is deprecated and insecure. "
                    "Migrate all templates to support CSP nonce. "
                    "Set SecurityHeaders(use_nonce=True)."
                )
