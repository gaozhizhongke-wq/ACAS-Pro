...
class TokenBlacklist:
    ...
    def _detect_backend(cls) -> str:
        ...
        cls._backend = "memory"
        logger.warning(
            "TokenBlacklist: using in-memory backend (tokens lost on restart)"
        )
        return "memory"
...
class JWTManager:
    ...
    def _get_secret_key() -> str:
        ...
        if not key:
            raise ValueError(
                "JWT secret key not configured. Set ACAS_JWT_SECRET env var or _cfg().security.secret_key"
            )
        return key

    @classmethod
    def generate_token(
        cls, user_id: str, extra_claims: Optional[Dict[str, Any]] = None
    ) -> str:
        ...
        return jwt.encode(  # type: ignore[no-any-return]
            payload, cls._get_secret_key(), algorithm=_cfg().security.jwt_algorithm
        )
    ...
    @classmethod
    def verify_token(
        cls, token: str, expected_type: str = "access"
        ...
        try:
            payload = jwt.decode(
                token, cls._get_secret_key(), algorithms=[_cfg().security.jwt_algorithm]
            )
    ...
    def __init__(self, storage_path: Optional[str] = None) -> None:
        if storage_path is None:
            storage_path = os.path.join(
                os.environ.get("ACAS_DATA_DIR", os.path.join(Path.home(), ".acas-pro")),
                ".rate_limit.json",
            )
    ...
    def _atomic(self) -> None:
        ...
        with open(lock_path, "w") as lf:
            if _WIN32:
                ...
                fcntl.flock(lf.fileno(), fcntl.LOCK_EX)  # type: ignore[name-defined]
                try:
                    yield
                finally:
...
def _get_rate_limiter() -> Any:
    ...
    return _get_lazy(
        "rate_limiter", None
    )  # handled specially  # type: ignore[arg-type]
...
if not CSRF_STATE_SECRET:
    if _cfg().is_production():
        from .exceptions import ACASConfigError

        raise ACASConfigError(
            "CSRF_STATE_SECRET must be set in production. "
            "Generate: python -c 'import secrets; print(secrets.token_hex(32))'"
        )
    import logging as _logging

    _logging.getLogger(__name__).warning(
        "CSRF_STATE_SECRET not set — critical for HMAC-based CSRF in production. "
        "Per-request tokens still work for development."
    )
...
def create_csrf_cookie(response) -> str:
    ...
    response.set_cookie(
        ...
        max_age=3600 * 24,
        httponly=False,
        secure=os.environ.get("ACAS_ENV") != "testing",
        samesite="Lax",
    )
    return token
