...
class TaobaoShopClient(PlatformAPIClient):
    ...
    def sync_orders(
        ...
        try:
            ...
            trades = (
                result.get("trades_sold_get_response", {})
                .get("trades", {})
                .get("trade", [])
            )
            total = result.get("trades_sold_get_response", {}).get("total_results", 0)
            return SyncResult(
                success=True, total=total, data=trades, created=len(trades)
            )
        except Exception as e:
            logger.exception("[淘宝] Order sync failed")
    ...
    def sync_products(self, page: int = 1) -> SyncResult:
        ...
        try:
            ...
            items = (
                result.get("items_onsale_get_response", {})
                .get("items", {})
                .get("item", [])
            )
            ...
            logger.exception("[淘宝] Product sync failed")
            return SyncResult(success=False, errors=[str(e)])
    ...
    def sync_inventory(self, product_ids: Optional[List[str]] = None) -> SyncResult:
        ...
        try:
            ...
            skus = (
                result.get("skus_custom_get_response", {})
                .get("skus", {})
                .get("sku", [])
            )
            return SyncResult(success=True, total=len(skus), data=skus)
        except Exception as e:
            logger.exception("[淘宝] Inventory sync failed")
            return SyncResult(success=False, errors=[str(e)])
    ...
    def update_product_status(self, product_id: str, status: str) -> bool:
        ...
        try:
            ...
            logger.exception("[淘宝] Update product status failed")
            return False
    ...
    def get_logistics_info(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            shipments = (
                result.get("logistics_orders_get_response", {})
                .get("shipments", {})
                .get("logistics_order", [])
            )
            return shipments[0] if shipments else {}
        except Exception as e:
            logger.exception("[淘宝] Logistics query failed")
            return {"error": str(e)}
    ...
    def exchange_token(self, code: str) -> Dict[str, str]:
        ...
        try:
            result = self.request("POST", "", data=params)
            token_data = result.get("top_auth_token_create_response", {}).get(
                "top_auth_token", {}
            )
            return {
            ...
            logger.exception("[淘宝] Token exchange failed")
            return {"error": str(e)}
