...
class Product:
    ...
    # 平台映射
    platform_mappings: Dict[str, str] = field(default_factory=dict)  # {platform: platform_product_id}
...
class ProductManager:
    ...
    def _save_product(self, product: Product) -> None:
        ...
        """, (
            ...
            product.category.value, product.sub_category,
            ...
            product.main_image, product.video_url,
            product.weight, product.length, product.width, product.height,
            product.status.value,
            json.dumps(product.platform_mappings),
            product.created_at, product.updated_at,
            product.owner_id, product.shop_id,
            product.total_sales, product.monthly_sales, product.weekly_sales
        ))
    ...
    def _row_to_product(self, row: Dict[str, Any]) -> Product:
        ...
        return Product(
            ...
            category=ProductCategory(row['category']) if row['category'] else ProductCategory.FASHION,
    ...
    def update_stock(self, product_id: str, quantity: int, variant_id: Optional[str] = None) -> bool:
        """更新库存"""
        product = self.get_product(product_id)
        if not product:
            return False
