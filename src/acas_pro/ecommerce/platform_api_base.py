...
class RateLimitError(APIError):
    ...
    def __init__(self, retry_after: int = 60):
        ...
        super().__init__(
            "RATE_LIMITED", f"API rate limited, retry after {retry_after}s"
        )
...
class PlatformAPIClient(ABC):
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                ...
                if resp.status_code == 401:
                    ...
                    raise APIError(
                        f"SERVER_{resp.status_code}", f"Server error: {resp.text[:200]}"
                    )
                elif resp.status_code >= 400:
                    raise APIError(
                        f"CLIENT_{resp.status_code}", f"Client error: {resp.text[:200]}"
                    )
    ...
    def _do_refresh_token(self) -> Optional[str]:
        """子类实现token刷新逻辑"""
        logger.warning(f"{self.PLATFORM_NAME} token refresh not implemented")
        return None
