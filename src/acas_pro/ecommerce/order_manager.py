...
class OrderManager:
    ...
    def _save_order(self, order: Order) -> None:
        ...
        """,
            (
                ...
                json.dumps(order.shipping_address.__dict__)
    ...
    def _row_to_order(self, row: Dict[str, Any]) -> Order:
        ...
        address_data = (
            json.loads(row["shipping_address"]) if row["shipping_address"] else None
        )
    ...
    def update_order_status(
        self, order_id: str, status: OrderStatus, note: Optional[str] = None
    ) -> bool:
        """更新订单状态"""
    ...
    def ship_order(
        self, order_id: str, logistics_company: str, tracking_no: str
    ) -> bool:
        """发货"""
        order = self.get_order(order_id)
        if not order:
    ...
    def sync_orders_from_platform(
        self, shop_id: str, platform: str, start_time: str, end_time: str
    ) -> Dict[str, Any]:
        """从平台同步订单
        ...
        if shop:
            ...
            if client and client.is_authenticated:
                try:
                    ...
                    if result.success:
                        # 将API返回的订单数据同步到本地
                        synced = self._merge_platform_orders(
                            result.data, shop_id, platform
                        )
                        logger.info(
                    ...
                    logger.exception(
                        "[OrderManager] Platform API sync failed, falling back to local data"
                    )

        # 回退到本地数据
        existing_orders = self.get_orders_by_shop(
            shop_id=shop_id, start_date=start_time, end_date=end_time, limit=1000
        )
    ...
    def get_order_statistics(
        ...
        """获取订单统计"""
        orders = self.get_orders_by_shop(
            shop_id, start_date=start_date, end_date=end_date, limit=10000
        )
    ...
    def search_orders(self, shop_id: str, keyword: str, limit: int = 50) -> List[Order]:
        """搜索订单"""
        # 搜索订单号、买家昵称、商品名称
