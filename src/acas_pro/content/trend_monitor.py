...
class TrendMonitor:
    ...
    def stop_monitoring(self) -> None:
        """停止监测"""
        self._running = False
        self._stop_event.set()  # 立即唤醒线程检查退出
    ...
    async def _monitor_loop_async(self) -> None:
        """异步监控主循环"""
        logger.info("[TrendMonitor] Started async monitoring loop")
    ...
    def _monitor_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            # 使用 Event.wait() 替代 time.sleep()，可在 stop 时立即唤醒
    ...
    def _analyze_content(self, item: TrendItem) -> None:
        """分析内容并计算评分"""
        # 计算爆款潜力指数
        engagement_rate = (item.likes + item.comments + item.shares) / max(
            item.views, 1
        )

        # 时间衰减因子
        hours_since_publish = (
            datetime.now() - item.publish_time
        ).total_seconds() / 3600
    ...
    def get_trending_items(
        ...
        for row in rows:
            item = TrendItem(
                ...
                visual_tags=json.loads(row["visual_tags"])
                if row["visual_tags"]
                else [],
            )
            items.append(item)
