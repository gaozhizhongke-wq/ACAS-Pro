...
class FestivalCalendar:
    ...
    def get_upcoming_festivals(
        self, days: int = 30, market: Optional[MarketType] = None
    ) -> List[Festival]:
        """获取即将到来的节日"""
        today = datetime.now()
    ...
    def get_marketing_plans(
        self, status: str = None, festival_id: Optional[str] = None
    ) -> List[MarketingPlan]:
        """获取营销计划列表"""
        ...
        for row in rows:
            plan = MarketingPlan(
                ...
                if isinstance(row["target_platforms"], str)
                else row["target_platforms"],
                target_accounts=json.loads(row["target_accounts"])
                if isinstance(row["target_accounts"], str)
                else row["target_accounts"],
                content_count=row["content_count"],
                content_types=json.loads(row["content_types"])
                if isinstance(row["content_types"], str)
                else row["content_types"],
                budget=row["budget"],
                status=row["status"],
                created_at=_parse_dt(row["created_at"]),
            )
