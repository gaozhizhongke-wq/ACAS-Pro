...
class DataMonitor:
    ...
    def aggregate_daily(self, platform: str, account_id: str, date: datetime):
        """聚合每日数据"""
    ...
    def generate_report(
        ...
        if report.total_views > 0:
            report.engagement_rate = (
                report.total_likes + report.total_comments + report.total_shares
            ) / report.total_views
        ...
        if prev_result and prev_result.get("views"):
            report.views_trend = (
                report.total_views - prev_result["views"]
            ) / prev_result["views"]
        if prev_result and prev_result.get("revenue"):
            report.revenue_trend = (
                report.total_revenue - prev_result["revenue"]
            ) / prev_result["revenue"]
    ...
    def get_alerts(
        self, acknowledged: bool = False, severity: str = None, limit: int = 100
    ) -> List[dict]:
        """获取预警列表"""
