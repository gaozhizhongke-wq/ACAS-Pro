...
class VoiceSynthesizer:
    ...
    def list_voices(
        self, language: Language = None, gender: str = None
    ) -> List[VoiceProfile]:
        """列出可用声音"""
    ...
    def batch_synthesize(
        self, texts: List[str], voice_id: str = "cn_female_01", **kwargs
    ) -> List[str]:
        """批量合成"""
        results = []
        for text in texts:
    ...
    def clone_voice(
        self, name: str, sample_paths: List[str], description: str = ""
    ) -> Optional[str]:
        """
        声音克隆
    ...
    def mix_with_music(
        ...
        logger.warning("Video mixing not integrated, returning None")
        return None
    ...
    def list_tasks(self, limit: int = 50) -> List[dict]:
        ...
        rows = self.db.fetchall(
            "SELECT * FROM voice_tasks ORDER BY created_at DESC LIMIT ?", (limit,)
        )
