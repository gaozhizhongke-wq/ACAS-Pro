...
def _init_metrics() -> None:
    ...
    _metrics['app_info'] = Info(
        'acas_pro', 'ACAS Pro application information',
        registry=_registry,
    )
    _metrics['request_count'] = Counter(
        'acas_pro_http_requests_total', 'Total HTTP requests',
    ...
    _metrics['llm_requests'] = Counter(
        'acas_pro_llm_requests_total', 'Total LLM API requests',
        ['provider', 'status'],
        registry=_registry,
    )
    _metrics['llm_tokens'] = Counter(
        'acas_pro_llm_tokens_total', 'Total LLM tokens consumed',
        ['provider', 'type'],
        registry=_registry,
    )
    _metrics['active_users'] = Gauge(
        'acas_pro_active_users', 'Number of active users',
        registry=_registry,
    )
    _metrics['db_connections'] = Gauge(
        'acas_pro_db_connections_active', 'Active database connections',
        registry=_registry,
    )
    _metrics['error_count'] = Counter(
        'acas_pro_errors_total', 'Total errors',
        ['type', 'endpoint'],
        registry=_registry,
    )
