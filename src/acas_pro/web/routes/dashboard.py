        ...
        try {
            ...
            if (statsRes.ok) {
                ...
                if (statsData.success && statsData.stats) {
                    ...
                    document.getElementById('content-count').textContent = s.content_count || '0';
                    document.getElementById('pending-tasks').textContent = s.pending_tasks || '0';
                    document.getElementById('api-calls').textContent = (s.api_calls_today || 0).toLocaleString();
                }
            }
            ...
            if (actRes.ok) {
                ...
                if (actData.success && actData.activities) {
                    ...
                    tbody.innerHTML = actData.activities.map(a => 
                        `<tr><td>${a.time}</td><td>${a.event}</td><td><span class="status-dot ${a.status === 'completed' || a.status === 'success' || a.status === 'info' ? 'healthy' : 'degraded'}"></span>${a.status}</td></tr>`
                    ).join('');
                }
            }
