...
class BiddingEngine:
    ...
    # 设备系数
    DEVICE_MULTIPLIERS = {"mobile": 1.0, "tablet": 0.9, "desktop": 0.8}
    ...
    def calculate_bid(self, config: BiddingConfig, context: Dict[str, Any]) -> float:
        """
        计算最终出价
        ...
        # 应用策略特定调整
        final_bid = self._apply_strategy_adjustment(config, final_bid, context)
    ...
    def _apply_strategy_adjustment(
        self, config: BiddingConfig, bid: float, context: Dict[str, Any]
    ) -> float:
        """应用策略特定调整"""
    ...
    def optimize_bidding(
        self, config: BiddingConfig, performance_data: List[Dict[str, Any]]
    ) -> BiddingConfig:
        """
        基于历史表现优化出价配置
        ...
        sum(d.get("cvr", 0) for d in performance_data) / len(performance_data)
        sum(d.get("spend", 0) for d in performance_data) / max(
            sum(d.get("clicks", 0) for d in performance_data), 1
        )
        ...
        if config.strategy == BiddingStrategy.TARGET_CPA and config.target_cpa:
            current_cpa = sum(d.get("spend", 0) for d in performance_data) / max(
                sum(d.get("conversions", 0) for d in performance_data), 1
            )
            ...
            if current_cpa > config.target_cpa * 1.2:
                ...
                self.logger.info(
                    f"CPA过高 ({current_cpa:.2f} > {config.target_cpa:.2f})，降低出价"
                )
            elif current_cpa < config.target_cpa * 0.8:
                # CPA有空间，可以提高出价
                new_config.base_bid *= 1.05
                self.logger.info(
                    f"CPA有优化空间 ({current_cpa:.2f} < {config.target_cpa:.2f})，适度提高出价"
                )

        # 根据CTR调整时段系数
        if avg_ctr < 0.01:
            # CTR过低，降低高峰时段出价
            new_config.adjustments.append(
                BidAdjustment(
    ...
    def get_bid_suggestion(
        self, platform: str, objective: str, target_audience_size: int
    ) -> Dict[str, Any]:
        """
        获取出价建议
        ...
        suggestion = base_suggestions.get(platform, {}).get(
            objective, {"min": 1.0, "suggested": 5.0, "max": 30.0}
        )
    ...
    def _get_strategy_recommendation(self, platform: str, objective: str) -> str:
        ...
        return recommendations.get(
            (platform, objective), "建议使用自动出价策略，根据数据表现调整"
        )

    def simulate_bidding(
        self, config: BiddingConfig, scenarios: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        模拟不同场景下的出价表现
        ...
        for scenario in scenarios:
            ...
            expected_impressions = (
                scenario.get("available_impressions", 1000) * win_probability
            )
