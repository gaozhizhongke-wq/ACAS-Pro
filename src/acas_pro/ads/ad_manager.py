...
class AdManager:
    ...
    def add_account(self, account: AdAccount) -> bool:
        ...
        try:
            ...
            encrypted_refresh = (
                encrypt_data(account.refresh_token) if account.refresh_token else None
            )
    ...
    def get_all_accounts(
        self, platform: Optional[AdPlatform] = None
    ) -> List[AdAccount]:
        """获取所有广告账户"""
        try:
            if platform:
                rows = self.db.fetchall(
                    "SELECT * FROM ad_accounts WHERE platform = ?", (platform.value,)
                )
            else:
    ...
    def get_campaigns(
        self, account_id: Optional[str] = None, status: Optional[CampaignStatus] = None
    ) -> List[AdCampaign]:
        """获取广告计划列表"""
        try:
            conditions = []
        ...
        try:
            ...
            if row and row.get("SUM(impressions)"):
                return {
                    ...
                    "cost_per_conversion": row.get("AVG(cost_per_conversion)", 0.0)
                    or 0.0,
                }
        except (
            sqlite3.Error,
    ...
    async def get_all_accounts_async(
        self, platform: Optional[AdPlatform] = None
    ) -> List[AdAccount]:
        """获取所有广告账户 (异步)"""
        try:
    ...
    async def update_account_balance_async(
    ...
    async def update_campaign_status_async(
    ...
    async def record_daily_stats_async(
        self,
        campaign_id: str,
        adset_id: str,
        date: str,
    ...
    async def get_campaign_stats_async(
    ...
    async def get_platform_comparison_async(
        self, days: int = 30
