...
def _row_to_segment(row: Dict[str, Any]) -> AudienceSegment:
    ...
    return AudienceSegment(
        ...
        age_range=AgeRange(**json.loads(row["age_range"]))
        if row.get("age_range")
        else AgeRange(),
        geo_targeting=GeoTargeting(**json.loads(row["geo_targeting"]))
        if row.get("geo_targeting")
        else GeoTargeting(),
        device_targeting=DeviceTargeting(**json.loads(row["device_targeting"]))
        if row.get("device_targeting")
        else DeviceTargeting(),
...
class AudienceTargeting:
    ...
    def __init__(
        self, db: Optional[DatabaseManager] = None, db_path: Optional[str] = None
    ):
    ...
    def get_segment(self, segment_id: str) -> Optional[AudienceSegment]:
        ...
        try:
            row = self.db.fetch_one(
                "SELECT * FROM audience_segments WHERE id = ?", (segment_id,)
            )
            if row:
    ...
    def get_segments(
        self, type: Optional[AudienceType] = None, status: Optional[str] = None
    ...
    def estimate_audience_size(self, segment: AudienceSegment) -> Dict[str, Any]:
        ...
        interest_coverage = (
            min(1.0, len(segment.interests) * 0.1) if segment.interests else 1.0
        )
        behavior_coverage = (
            min(1.0, len(segment.behaviors) * 0.15) if segment.behaviors else 1.0
        )
    ...
    def create_lookalike(
        self, source_segment_id: str, name: str, ratio: float = 0.01
        ...
        lookalike = AudienceSegment(
            ...
            estimated_daily_impressions=int(
                source.estimated_daily_impressions * (1 + ratio * 5)
            ),
        )
    ...
    def get_recommended_targeting(
        self, product_category: str, target_platform: str
    ) -> Dict[str, Any]:
        """获取推荐定向配置"""
        recommendations = {
            "美妆": {
    ...
    async def create_lookalike_async(
        self, source_segment_id: str, name: str, ratio: float = 0.01
    ) -> Optional[AudienceSegment]:
        """创建相似人群 (异步)"""
