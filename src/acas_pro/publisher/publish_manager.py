...
class PublishManager:
    ...
    def create_task(
        ...
        task = PublishTask(
            ...
            tags=tags or [],
            platforms=[
                PlatformConfig(platform=p, account_id="")
                for p in (platforms or ["douyin"])
            ],
    ...
    def _save_task(self, task: PublishTask) -> None:
        ...
        """,
            (
                ...
                task.cover_image,
                json.dumps(
                    [self._platform_config_to_dict(p) for p in task.platforms],
                    ensure_ascii=False,
                ),
    ...
    def _row_to_task(self, row: dict) -> PublishTask:
        ...
        return PublishTask(
            ...
            publish_results=json.loads(row["publish_results"])
            if row["publish_results"]
            else {},
            created_at=datetime.fromisoformat(row["created_at"]),
            published_at=datetime.fromisoformat(row["published_time"])
    ...
    def adapt_content_for_platform(
        self, title: str, description: str, tags: List[str], platform: str
    ) -> Dict[str, any]:
        """
        根据平台特性适配内容
        ...
        # 截断标题
        adapted_title = (
            title[: features["title_max"]] if features["title_max"] > 0 else ""
        )
    ...
    def list_tasks(
        self, status: PublishStatus = None, platform: str = None, limit: int = 50
    ) -> List[PublishTask]:
        """列出任务"""
        ...
        if platform:
            tasks = [
                t for t in tasks if any(p.platform == platform for p in t.platforms)
            ]
