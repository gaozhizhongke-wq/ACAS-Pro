...
class AccountManager:
    ...
    def add_account(
        ...
        **kwargs
        ...
        encrypted_refresh = self.security.encrypt(refresh_token) if refresh_token else None
    ...
    def _save_account(self, account: PlatformAccount):
        ...
        """, (
            account.id, account.platform.value, account.account_id,
            account.account_name, account.nickname,
            account.access_token, account.refresh_token, account.token_expires_at.isoformat(),
            account.avatar_url, account.followers, account.following,
            account.total_likes, account.total_views, account.content_count,
            account.status.value, account.phase.value,
            json.dumps(account.tags, ensure_ascii=False),
            account.region, account.category,
            account.risk_score,
            account.last_violation_at.isoformat() if account.last_violation_at else None,
    ...
    def get_access_token(self, account_id: str) -> Optional[str]:
        """获取解密的访问令牌"""
        row = self.db.fetchone(
            "SELECT access_token FROM platform_accounts WHERE id = ?",
            (account_id,)
        )
