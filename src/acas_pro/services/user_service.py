...
class UserService:
    ...
    def register(
        ...
        existing = _get_lazy_db().fetchone(
    ...
    def login(
        self, account: str, password: str, ip_address: str = None
    ) -> Tuple[bool, str, Optional[UserProfile]]:
        """
        Authenticate user
        ...
        if not _get_lazy_rate_limiter().is_allowed(
            rate_key, max_attempts=5, window_seconds=300
        ):
            audit_logger.log(
                "LOGIN_RATE_LIMITED",
                account,
        ...
        if not _get_lazy_password_hasher().verify(password, user["password_hash"]):
            ...
            if failed_count >= 5:
                ...
                _get_lazy_db().update(
                    "users", {"failed_login_count": failed_count}, {"id": user["id"]}
                )
