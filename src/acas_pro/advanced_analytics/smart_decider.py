...
class SmartDecider:
    ...
    def _analyze_seasonal_metrics(self, metrics: Dict[str, Any]) -> list:
        ...
        if events:
            decisions.append(Decision(
                ...
                confidence=0.75,
                expected_impact=0.3,
                action_plan=[f"Prepare campaign for {e.get('name', 'event')}" for e in events],
                related_channels=list(set(ch for e in events for ch in e.get("channels", []))),
                related_products=list(set(p for e in events for p in e.get("products", []))),
    ...
    def complete_decision(self, decision_id: str, actual_impact: float = 0.0, notes: str = "") -> bool:
        for d in self._decisions:
            if d.decision_id == decision_id:
    ...
    def generate_report(self, period_start=None, period_end=None, start=None, end=None) -> DecisionReport:
        ...
        in_range = [
            ...
            if d.created_at and s <= d.created_at <= e
        ]
