...
class GestureGenerator:
    ...
    def generate_gestures_for_script(
        self, script: str, duration: float, style: str = "natural"
    ) -> List[Gesture]:
        """根据脚本生成手势序列"""
        gestures = []
        current_time = 0.0
    ...
    def _select_gesture_for_segment(
        self, segment: Dict[str, Any], style: str
    ) -> Optional[Gesture]:
        """为段落选择合适的手势"""
        type_mapping = {
        ...
        # 查找匹配的手势
        matching = [g for g in self._gesture_library.values() if g.type == gesture_type]
    ...
    def get_gestures_by_type(self, gesture_type: GestureType) -> List[Gesture]:
        """按类型获取手势"""
        return [g for g in self._gesture_library.values() if g.type == gesture_type]

    def interpolate_pose(
        self, pose1: PoseFrame, pose2: PoseFrame, t: float
    ) -> PoseFrame:
        """插值两个姿态"""
        ...
        # 插值所有关节
        joint_rotations = {}
        all_parts = set(pose1.joint_rotations.keys()) | set(
            pose2.joint_rotations.keys()
        )
        ...
        return PoseFrame(
            ...
            facial_expression=pose1.facial_expression
            if t < 0.5
            else pose2.facial_expression,
            expression_intensity=lerp(
                pose1.expression_intensity, pose2.expression_intensity, t
            ),
        )
