...
class SceneAdapter:
    ...
    def create_scene_from_template(
        ...
        template = self.SCENE_TEMPLATES.get(
            scene_type, self.SCENE_TEMPLATES[SceneType.PRODUCT_SHOWCASE]
        )
        ...
        scene = SceneConfig(
            ...
            lighting_config=self.LIGHTING_PRESETS.get(
                template["lighting_preset"], LightingConfig()
            ),
        ...
        if customizations:
            ...
            if "lighting_preset" in customizations:
                ...
                scene.lighting_config = self.LIGHTING_PRESETS.get(
                    preset, LightingConfig()
                )
    ...
    def get_scenes_by_type(self, scene_type: SceneType) -> List[SceneConfig]:
        """按类型获取场景"""
        return [s for s in self._custom_scenes.values() if s.scene_type == scene_type]
    ...
    def adapt_avatar_to_scene(
        self, avatar_id: str, scene_id: str, content_type: str = "standard"
    ) -> Dict[str, Any]:
        """将数字人适配到场景"""
        scene = self.get_scene(scene_id)
        if not scene:
    ...
    def suggest_scene_for_content(
        self, content_description: str, target_platform: Optional[str] = None
    ) -> List[SceneType]:
        """为内容推荐场景"""
