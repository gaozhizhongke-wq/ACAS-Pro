...
class VideoLogic:
    ...
    def get_quality_settings(self, quality: VideoQuality) -> Dict:
        ...
        return self.QUALITY_SETTINGS.get(
            quality, self.QUALITY_SETTINGS[VideoQuality.FHD_1080P]
        )
    ...
    def validate_project(self, project_id: str) -> List[str]:
        ...
        if total_scene_duration != project.duration:
            issues.append(
                f"Scene durations ({total_scene_duration}s) don't match project duration ({project.duration}s)"
            )
