...
class DashboardLogic:
    ...
    def calculate_kpis(self, data: Optional[dict] = None) -> List[KPIData]:
        ...
        revenue_trend = ((revenue - revenue_prev) / revenue_prev * 100) if revenue_prev else 0
...
subtitle=f"较上月{self._format_trend(revenue_trend)}",
        ...
        orders_trend = ((orders - orders_prev) / orders_prev * 100) if orders_prev else 0
...
subtitle=f"较上月{self._format_trend(orders_trend)}",
        ...
        kpis.append(KPIData(
            ...
            color=self.COLORS['warning'] if low_stock > 0 else self.COLORS['success'],
        ...
        if total_alerts > 0:
            ...
            alert_color = self.COLORS['danger'] if critical > 0 else self.COLORS['warning']
