...
from ...avatar.avatar_engine import AvatarEngine, DigitalAvatar, AvatarType, AvatarStyle
from ...avatar.scene_adapter import SceneAdapter, SceneType
...
class CreateAvatarDialog(QDialog):
    ...
    def setup_ui(self) -> None:
        ...
        self.age_combo.addItems(
            ["年轻(18-25)", "中年(26-35)", "成熟(36-45)", "资深(46+)"]
        )
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if config["method"] == "template":
                avatar = self.engine.create_avatar_from_template(
                    config["template_id"], name="我的数字人"
                )
                if avatar:
                    QMessageBox.information(
                        self, "成功", f"数字人 '{avatar.name}' 创建成功！"
                    )
                    self.load_avatars()
            else:
