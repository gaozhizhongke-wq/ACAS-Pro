...
class SettingsPage(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        # 通知设置
        tabs.addTab(
            self._create_notification_tab(), t("settings.notifications", "通知")
        )
    ...
    def _create_security_tab(self) -> QWidget:
        ...
        session_layout.addRow(
            t("settings.session_timeout", "会话超时(分钟)"), self.session_timeout
        )
        ...
        session_layout.addRow(
            t("settings.max_login_attempts", "最大登录尝试次数"), self.max_attempts
        )
    ...
    def _create_llm_tab(self) -> QWidget:
        ...
        help_label.setStyleSheet(
            f"color: {COLORS['text2']}; font-size: 12px; padding: 12px; background: {COLORS['surface']}; border-radius: 6px;"
        )
    ...
    def _load_llm_settings(self) -> None:
        ...
        try:
            ...
            self.llm_agent_mode.setChecked(
                config.llm.agent_mode if hasattr(config.llm, "agent_mode") else True
            )
            self.llm_max_steps.setValue(
                config.llm.max_agent_steps
                if hasattr(config.llm, "max_agent_steps")
                else 10
            )
    ...
    def _create_oauth_tab(self) -> QWidget:
        ...
        self.wx_redirect.setPlaceholderText(
            "https://acas-pro.com/oauth/callback/wechat"
        )
