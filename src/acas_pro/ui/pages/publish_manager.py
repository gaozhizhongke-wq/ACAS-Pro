...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QDateTimeEdit, QTabWidget, QSpinBox
)
...
class PublishManagerPage(QWidget):
    ...
    def _create_tasks_tab(self) -> QWidget:
        ...
        self.status_filter.addItems(["全部", "待发布", "已排期", "发布中", "已发布", "失败"])
    ...
    def _create_new_tab(self) -> QWidget:
        ...
        immediate_checkbox.toggled.connect(lambda checked: self.datetime_edit.setEnabled(not checked))
    ...
    def _load_tasks(self) -> None:
        ...
        for task in tasks:
            ...
            platforms_str = ", ".join([platform_names.get(p.platform, p.platform) for p in task.platforms[:3]])
    ...
    def _create_task(self) -> None:
        ...
        if scheduled_time is None:
            ...
            QMessageBox.information(self, "成功", f"发布任务已排期: {scheduled_time.strftime('%Y-%m-%d %H:%M')}")
