...
class DecisionCard(QFrame):
    ...
    def _init_ui(self) -> None:
        ...
        status_label.setStyleSheet(
            f"color: {status_colors.get(status, COLORS['gray'])};"
        )
    ...
    def _run_attribution_analysis(self) -> None:
        ...
        for i, (channel, weight) in enumerate(sample_data.items()):
            ...
            self.channel_table.setItem(
                i, 2, QTableWidgetItem(str(random.randint(100, 500)))
            )
            self.channel_table.setItem(
                i, 3, QTableWidgetItem(f"¥{random.randint(10000, 50000)}")
            )
            self.channel_table.setItem(
                i, 4, QTableWidgetItem(f"¥{random.randint(1000, 5000)}")
            )
            self.channel_table.setItem(
                i, 5, QTableWidgetItem(f"{random.uniform(2, 5):.2f}")
            )
            self.channel_table.setItem(i, 6, QTableWidgetItem(f"{weight:.1f}%"))
            self.channel_table.setItem(
                i, 7, QTableWidgetItem(f"{random.uniform(70, 95):.0f}%")
            )
    ...
    def _compare_models(self) -> None:
        ...
        for i, model in enumerate(models):
            ...
            self.compare_table.setItem(
                i, 1, QTableWidgetItem(str(random.randint(400, 600)))
            )
            self.compare_table.setItem(
                i, 2, QTableWidgetItem(f"¥{random.randint(200000, 400000)}")
            )
            self.compare_table.setItem(
                i, 3, QTableWidgetItem(f"¥{random.randint(50000, 100000)}")
            )
            self.compare_table.setItem(
                i, 4, QTableWidgetItem(f"{random.uniform(2.5, 4.5):.2f}")
            )
