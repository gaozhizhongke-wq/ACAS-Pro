...
class FestivalCard(QFrame):
    ...
    def _setup_ui(self) -> None:
        ...
        # 持续时间和预热
        duration = QLabel(
            f"⏱ 持续 {self.festival.duration_days} 天 | 预热 {self.festival.pre_heat_days} 天"
        )
...
class FestivalCalendarPage(QWidget):
    ...
    def _create_all_festivals_tab(self) -> QWidget:
        ...
        self.festivals_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
    ...
    def _load_all_festivals(self) -> None:
        ...
        for i, festival in enumerate(festivals):
            ...
            self.festivals_table.setItem(
                i, 2, QTableWidgetItem(type_map.get(festival.festival_type, ""))
            )
    ...
    def _load_plans(self) -> None:
        ...
        for i, plan in enumerate(plans):
            ...
            self.plans_table.setItem(
                i, 6, QTableWidgetItem(status_map.get(plan.status, plan.status))
            )
