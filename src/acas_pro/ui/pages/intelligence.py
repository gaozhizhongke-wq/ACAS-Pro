...
class IntelligencePage(QWidget):
    ...
    def _create_news_tab(self) -> QWidget:
        ...
        self.cat_combo.addItems(
            ["All", "Business", "Technology", "Finance", "Logistics", "Disaster"]
        )
    ...
    def _create_alerts_tab(self) -> QWidget:
        ...
        self.wechat_input.setPlaceholderText(
        ...
        self.dingtalk_input.setPlaceholderText(
    ...
    def _create_settings_tab(self) -> QWidget:
        ...
        rss_info = QLabel(
            "RSS sources are configured in collectors/rss_collector.py\n"
    ...
    def _load_intelligence(self) -> None:
        ...
        try:
            ...
            for i, article in enumerate(articles):
                self.table.setItem(
                    i, 0, QTableWidgetItem(article.published_at.strftime("%m-%d %H:%M"))
                )
                self.table.setItem(
                    i, 1, QTableWidgetItem(article.category.value.title())
                )
                ...
                if article.sentiment:
                    sentiment_text = article.sentiment.overall_sentiment.value.replace(
                        "_", " "
                    ).title()
                ...
                self.table.setItem(
                    i, 5, QTableWidgetItem(f"{article.relevance_score:.0%}")
                )
    ...
    def _update_reputation(self) -> None:
            ...
            if article.sentiment:
                sentiment_articles.append(
                    SentimentArticle(
                        ...
                        if hasattr(article.sentiment, "score")
                        else 0,
                        sentiment_level=self._map_sentiment_level(
                            article.sentiment.overall_sentiment.value
                        ),
        ...
        if sentiment_articles:
            self._reputation_score = reputation_calculator.calculate(
                sentiment_articles,
                previous_score=float(self.main_score.text())
                if self.main_score.text() != "--"
                else None,
            )
            ...
            self.positive_count.setText(
                f"{self._reputation_score.positive_count} positive"
            )
            self.neutral_count.setText(
                f"{self._reputation_score.neutral_count} neutral"
            )
            self.negative_count.setText(
                f"{self._reputation_score.negative_count} negative"
            )

            trend_icon = (
                "↑"
                if self._reputation_score.trend == "improving"
                else "↓"
                if self._reputation_score.trend == "declining"
                else "→"
            )
            self.trend_label.setText(
                f"{trend_icon} {self._reputation_score.trend.title()}"
            )
            ...
            self.platform_table.setRowCount(
                len(self._reputation_score.platform_breakdown)
            )
            for i, (platform, score) in enumerate(
                self._reputation_score.platform_breakdown.items()
            ):
