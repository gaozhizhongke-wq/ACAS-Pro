...
from ...ads.audience_targeting import AudienceTargeting
...
class CampaignDialog(QDialog):
    ...
    def __init__(
        self, manager: AdManager, campaign: Optional[AdCampaign] = None, parent=None
    ):
    ...
    def load_campaign_data(self) -> None:
        ...
        self.platform_combo.setCurrentIndex(platform_map.get(self.campaign.platform, 0))
    ...
    def get_campaign_data(self) -> Dict[str, Any]:
        """获取广告计划数据"""
        platforms = [
            AdPlatform.OCEAN_ENGINE,
            AdPlatform.MAGNETIC_ENGINE,
            AdPlatform.TENCENT_ADS,
            AdPlatform.KUAISHOU_ADS,
        ...
        strategies = [
            ...
            BiddingStrategy.AUTO_OCPC,
            BiddingStrategy.AUTO_OCPM,
            BiddingStrategy.MAX_CONVERSION,
            BiddingStrategy.TARGET_CPA,
            BiddingStrategy.TARGET_ROI,
        ]
        ...
        return {
            ...
            if self.budget_type_combo.currentIndex() == 0
            else BudgetType.TOTAL,
...
class AdManagerPage(QWidget):
    ...
    def _create_campaign_tab(self) -> QWidget:
        ...
        self.total_spend_label.setStyleSheet(
            "font-size: 16px; font-weight: bold; color: #4361ee;"
        )
        stats_layout.addWidget(self.total_spend_label)

        self.total_impressions_label = QLabel("总展示: 0")
        self.total_impressions_label.setStyleSheet(
    ...
    def load_accounts(self) -> None:
        ...
        for account in accounts:
            ...
            items = [
                ...
                f"¥{account.daily_budget_limit:,.2f}"
                if account.daily_budget_limit > 0
                else "无限制",
                "编辑 | 刷新余额 | 删除",
            ]
    ...
    def on_segment_selected(self, item) -> None:
        ...
        if segment:
            ...
            self.segment_status_label.setText(
                '正常" if segment.status == "active" else "暂停'
            )
