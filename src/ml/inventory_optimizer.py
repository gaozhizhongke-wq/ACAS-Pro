...
from acas_pro.ml.timesfm_engine import timesfm_engine, ForecastResult
...
class InventoryRecommendation:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "days_until_stockout": round(self.days_until_stockout, 1) if self.days_until_stockout else None,
...
class InventoryOptimizer:
    ...
    def _analyze_product(
        ...
        if len(history) >= 7:
            daily_demand = [v for _, v in history[-30:]] if len(history) >= 30 else [v for _, v in history]
            avg_demand = mean(daily_demand)
            demand_std = stdev(daily_demand) if len(daily_demand) > 1 else avg_demand * 0.2
        else:
        ...
        if len(history) >= 14:
            ...
            forecasted_demand = sum(p.value for p in forecast.forecast)
            demand_trend = forecast.trend_direction
        else:
            forecasted_demand = avg_demand * forecast_days
            demand_trend = "stable"
        ...
        if annual_demand > 0 and unit_cost > 0:
            ...
            eoq = int((2 * annual_demand * self.ordering_cost / holding_cost_per_unit) ** 0.5)
        ...
        if current_stock <= safety_stock:
            ...
            urgency = "high"
            order_qty = max(int(eoq * 0.8), reorder_point - current_stock + int(safety_stock * 0.5))
            reasoning = f"库存低于再订货点({reorder_point})。建议尽快补货以避免缺货风险。"
    ...
    def assess_stockout_risks(
        self,
        inventory_data: List[Dict],
        sales_forecasts: Dict[str, ForecastResult]
    ) -> List[StockoutRisk]:
        ...
        for item in inventory_data:
            ...
            if stockout_day is None:
                ...
                revenue_at_risk = sum(p.value for p in forecast.forecast[stockout_day:]) * unit_price
    ...
    def calculate_inventory_metrics(self, recommendations: List[InventoryRecommendation]) -> Dict:
