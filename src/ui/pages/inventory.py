...
from acas_pro.ml.inventory_optimizer import inventory_optimizer
...
class InventoryPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels([
            "Product", "Current Stock", "Recommended Order", "Urgency",
            "Days Until Stockout", "Reorder Point", "Confidence"
        ])
    ...
    def _generate_recommendations(self):
        ...
        inventory_data = [
            {"product_id": "PROD-001", "name": "Premium Widget A", "stock": 150, "cost": 25.0},
            {"product_id": "PROD-002", "name": "Standard Widget B", "stock": 45, "cost": 15.0},
            {"product_id": "PROD-003", "name": "Deluxe Widget C", "stock": 8, "cost": 50.0},
            {"product_id": "PROD-004", "name": "Basic Widget D", "stock": 500, "cost": 10.0},
            {"product_id": "PROD-005", "name": "Pro Widget E", "stock": 23, "cost": 75.0},
        ]
        ...
        for i, rec in enumerate(recommendations):
            ...
            self.table.setItem(i, 2, QTableWidgetItem(str(rec.recommended_order_quantity)))
            self.table.setItem(i, 3, QTableWidgetItem(rec.urgency_level.upper()))
            
            days_str = f"{rec.days_until_stockout:.1f}" if rec.days_until_stockout else "N/A"
        ...
        if critical_count > 0:
            self.alert_banner.setText(f"⚠️ {critical_count} product(s) require immediate reorder!")
